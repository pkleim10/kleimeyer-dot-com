// Test script for opening move 21
// Using built-in fetch (Node.js 18+)

async function testOpeningMove21() {
  const xgid = '-b----E-C---eE---c-e----B-:0:0:1:21:0:0:0:0:10';

  console.log('Testing opening move 21...');
  ...
  try {
    const response = await fetch('http://localhost:3000/api/backgammon-engine', {
      ...
      body: JSON.stringify({
        xgid,
        player: 1,
        maxTopMoves: 6  // Explicitly set to match current config
      }),
    });
