// Test script for opening move 61
// Using built-in fetch (Node.js 18+)

async function testOpeningMove61() {
  const xgid = '-b----E-C---eE---c-e----B-:0:0:1:61:0:0:0:0:10';

  console.log('Testing opening move 61...');
  ...
  try {
    const response = await fetch('http://localhost:3000/api/backgammon-engine', {
      ...
      body: JSON.stringify({
        xgid,
        player: 1,
        maxTopMoves: 10,  // Include all 10 legal moves for MC analysis
        numSimulations: 1000,  // Full MC simulation with 1000 games per move
        debug: true
      }),
    });
